"""
Summarise ``data/fake_news_final.tsv`` (option-1 gated multimodal cohort) for reports / thesis.
...
def _markdown_option1_methodology() -> str:
    """Static description aligned with ``11_cohort_validate_images_option1.py`` (keep in sync manually)."""
    return "\n".join(
        [
            "## How `image_option1_validity_score` is computed (option-1)\n",
    ...
    return {
        "internal_project_documents": [
            ...
            },
            {
                ...
                "note": "Optional local notes (gitignored by default): cohort fetch scope, option-1 threshold, training eligibility decisions.",
            },
            {
            ...
            {
                "path": "outputs/cohort_image_validation/option1_validation.tsv",
        ...
        "literature_context_note": (
            ...
            "project's composite entropy/aspect/resolution score. Treat option-1 as an explicit, reproducible "
        ...
        "illustrative_related_work": (
            ...
            "work motivates rigorous data handling but is not the source of the option-1 formula."
        ),
        "external_paper_for_exact_option1_score": None,
    }
...
def _markdown_references_section() -> str:
    ...
    lines.append("")
    lines.append("### Illustrative related work (not the source of the option-1 formula)")
...
def main() -> int:
    ...
    summary = {
        ...
        "option1_methodology_reference": OPTION1_SCRIPT,
    ...
    lines.append(f"- **Rows:** **{n:,}** (option-1 gate: score ≥ 75, successful cohort fetch + QC)")
    lines.append(f"- **Score implementation:** `{OPTION1_SCRIPT}`")
    lines.append("")
    lines.append(_markdown_option1_methodology())
