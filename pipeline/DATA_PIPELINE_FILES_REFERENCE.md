...
|-------|------|------------------|
| 03 consolidate | `pipeline/05_consolidate_fakenews_tsv.py` | Build `data/fakenews.tsv` from Fakeddit multimodal TSVs + FNN `news content.json` (+ optional `crawl_failures.jsonl` filter). |
...
| 02 FNN QA | `pipeline/04_qa_fnn_visualize_crawl_failures.py` | PNGs under `outputs/fnn_failure_viz/`. |
| 04 optional | `pipeline/06_images_full_table_fetch.py` | Optional full-table image fetch (working TSV under `data/processed/images/`). |
| 05 cohort | `pipeline/07_cohort_build_plan.py` | Build stratified cohort plan TSV under `data/processed/cohorts/`. |
| 05 cohort | `pipeline/08_cohort_fetch_images.py` | Cohort image fetch → `cohort_image_fetch.log` + image files. |
| 05 cohort | `pipeline/08_cohort_reddit_placeholder_sha256.txt` | SHA-256 blocklist for `08_cohort_fetch_images.py`. |
| 05 cohort | `pipeline/09_cohort_dedupe_fetch_log.py` | Dedupe `cohort_image_fetch.log`. |
| 05 cohort | `pipeline/10_cohort_summarize_fetch.py` | `outputs/cohort_fetch_report/*.md` + `*.json`. |
| 05 cohort | `pipeline/11_cohort_validate_images_option1.py` | Option-1 QC → `outputs/cohort_image_validation/`. |
| 05 cohort | `pipeline/12_cohort_merge_option1_into_fakenews.py` | Merge option-1 QC columns into `data/fakenews.tsv`. |
| 05 cohort | `pipeline/13_cohort_merge_fetch_log_into_fakenews.py` | Merge fetch columns into `data/fakenews.tsv`. |
| 05 cohort | `pipeline/14_cohort_export_final_tsv.py` | Write `data/fake_news_final.tsv` from `fakenews.tsv`. |
| 05 cohort | `pipeline/15_cohort_summarize_final.py` | `outputs/fake_news_final_report/*.md` + `*.json`. |
| 06 EDA | `pipeline/16_eda_visualize_fakenews_tsv.py` | Charts / `outputs/fakenews_viz/` from `fakenews.tsv`. |
