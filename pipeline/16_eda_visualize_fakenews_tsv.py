...
Requires: pandas, matplotlib (see ``requirements.txt``).
    ...
    python pipeline/16_eda_visualize_fakenews_tsv.py
    python pipeline/16_eda_visualize_fakenews_tsv.py --input data/fakenews.tsv --out-dir outputs/fakenews_viz

Paths are resolved from the **project root** (parent of ``pipeline/``).
"""
...
<body>
  ...
  <p>Source: <code>{src.name}</code> — {n:,} rows. Generated by <code>pipeline/16_eda_visualize_fakenews_tsv.py</code>.</p>
