...
def main() -> int:
    ...
    ap.add_argument(
        "--sort-only",
        action="store_true",
        help="Only rewrite cohort_image_validation.tsv sorted by validity_score (lowest first); no image IO",
    )
    args = ap.parse_args()
    ...
    tsv_path = out_dir / "cohort_image_validation.tsv"
    summ_path = out_dir / "cohort_image_validation_summary.log"
    ...
    with tsv_path.open(out_mode, encoding="utf-8", newline="") as fp:
        ...
        with log_path.open(encoding="utf-8", newline="") as log_fp:
            ...
            for row in reader:
                ...
                # Log paths are project-relative with forward slashes; do not convert to backslashes on POSIX
                # (that would make Path join a single wrong filename under PROJECT_ROOT).
                rel = (row.get("local_path") or "").strip()
                if not rel:
                    skipped_missing += 1
                    continue
                rel_out = Path(rel).as_posix()
    ...
    lines = [
        ...
        "",
        "validity_score is a heuristic 1-100 (higher ~ more photo-like under these rules).",
        "",
    ]
    if scores:
        arr = sorted(scores)
